...
struct AnimatedLogo {
    scene: vello::Scene,
    /// Offset from the window center, in pixels (y points down).
    position: Vec2,
    rotation: f64,
    velocity: Vec2,
    rotation_speed: f64,
...
fn main() {
    ...
    // Rotate and scale each logo around the center of its bounding box
    let bbox = PathTransformer::new(rust_logo_svg_path.clone()).to_box(None);
    ...
    let mut logos: Vec<AnimatedLogo> = (0..10)
        .map(|i| {
            let angle = (i as f64) * std::f64::consts::TAU / 10.0; // Distribute in circle
            let radius = 200.0;
            ...
            let velocity = match i as usize % 5 {
                0 => Vec2::new(50.0 + i * 10.0, 30.0 + i * 5.0), // Linear movement
                1 => Vec2::new(-40.0 - i * 8.0, 60.0 + i * 7.0), // Different linear
                2 => Vec2::new(80.0 + i * 12.0, -50.0 - i * 6.0), // Another linear
                3 => Vec2::new(-70.0 - i * 9.0, -40.0 - i * 4.0), // Diagonal
                _ => Vec2::new(45.0 + i * 11.0, 70.0 + i * 8.0), // Default
            };
            ...
            AnimatedLogo {
                ...
                rotation: 0.0,
                velocity,
                rotation_speed: (i + 1.0) * 0.3, // Different rotation speeds
                scale_oscillation: 0.1 + i * 0.02, // Different scale oscillation
                scale_phase: i * std::f64::consts::PI / 5.0, // Different phases
            }
        })
        .collect();
    ...
    common::run(
        ...
        move |scene, frame| {
            ...
            for logo in logos.iter_mut() {
                ...
                logo.position += logo.velocity * frame.dt;
                logo.rotation += logo.rotation_speed * frame.dt;

                // Simple screen wrapping
                let (half_w, half_h) = (frame.width / 2.0, frame.height / 2.0);
                if logo.position.x > half_w {
                    logo.position.x = -half_w;
                }
                if logo.position.x < -half_w {
                    logo.position.x = half_w;
                }
                if logo.position.y > half_h {
                    logo.position.y = -half_h;
                }
                if logo.position.y < -half_h {
                    logo.position.y = half_h;
                }
                ...
                let scale_factor =
                    1.0 + logo.scale_oscillation * (frame.elapsed + logo.scale_phase).sin();
                let base_scale = 0.3 + (logo.position.x.abs() + logo.position.y.abs()) * 0.0001; // Vary base scale slightly

                let transform = Affine::translate(center + logo.position)
                    * Affine::rotate(logo.rotation)
                    * Affine::scale(scale_factor * base_scale)
                    * Affine::translate(-logo_center);
                scene.append(&logo.scene, Some(transform));
            }
        },
    );
}
