...
fn main() {
    // Create rough Rust logo using Vello
    ...
    let bbox_path = PathTransformer::new(rust_logo_svg_path);
    let bbox = bbox_path.to_box(None);

    // Draw the rough logo into its own scene once
    let mut logo_scene = vello::Scene::new();
    rust_logo_svg_path_drawing.draw(&mut logo_scene);

    common::run(
        "rough_vello: rust logo",
        Color::from_rgb8(150, 192, 183),
        false,
        move |scene, frame| {
            // Center the logo's bounding box in the window
            let offset = Affine::translate((
                (frame.width - bbox.width()) / 2.0 - bbox.min_x.unwrap_or(0.0),
                (frame.height - bbox.height()) / 2.0 - bbox.min_y.unwrap_or(0.0),
            ));
            scene.append(&logo_scene, Some(offset));
        },
    );
}
